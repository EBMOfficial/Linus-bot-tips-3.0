 ...
 client.on('message', async message => { 
  if (message.content === (`${prefix}setReminder`)) {
    ...
    {name: `Hi ${message.author.username}`, value: "Please check back every two hours to bump the server."}
        ...
        message.channel.send(ReminderConfirmationEmbed).then(console.log(`${message.author.username} has set the 2 hour Disboard bump reminder on ${currentDate.toLocaleString()}.`))
    } else return;
  }
 })
 client.on('message', async message => {
  ...
  if (channel) {
   ...
   if (message.content === `!d bump`) {
    ...
    setTimeout(() => message.channel.send(`${BumpRole} chop-chop! It's time to bump! 😃`), 7200000);
      message.channel.send(BumpConfirmationEmbed).then(console.log(`${message.author.username} has bumped the server on ${currentDate.toLocaleString()}. The timer will now reinitiate.`))
    }
  } else return;
