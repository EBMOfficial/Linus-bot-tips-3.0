...
const BumpRoleCreationErrorEmbed = new Discord.MessageEmbed()
	...
	.then(collected => {
		...
		if (reaction.emoji.name === '✅') { 
            if (message.guild.roles.cache.find(role => role.name === "bumping ping")) return message.channel.send(BumpRoleCreationErrorEmbed).then(message => message.delete(10000)).then(console.log(`${message.author.username} tried to create a bumping ping role, but it turns out that the role already exists in the guild! Timestamp - ${currentDate.toLocaleString()}.`))
          ...
          .setFooter("For administrators, by EBMOfficial and the magic of discord.js.")
			message.channel.send(BumpRoleCreationExitEmbed).then(message => message.delete(10000));
...
client.on('message', async message => {
 ...
 if (message.content === `${prefix}giveBumpRole`) {
   ...
   } else message.member.roles.add(BumpRole).then(message.channel.send(BumpRoleEmbed)).then(message => message.delete(10000)).then(console.log(`${message.author.username} has been given the bumping ping role by me on ${currentDate.toLocaleString()}.`))
