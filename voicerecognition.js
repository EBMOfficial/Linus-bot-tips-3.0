...
const PREFIX = '^';
const _CMD_HELP        = PREFIX + 'support';
...
const _CMD_SNEEZE      = PREFIX + 'sneeze';
const _CMD_RICKROLL    = PREFIX + 'rickroll';
const _CMD_RINGSWOW    = PREFIX + 'wow';
const _CMD_METAL       = PREFIX + 'metal';
const _CMD_JOOBIDIN    = PREFIX + 'joe biden';
const _CMD_KANYE       = PREFIX + 'kanye';
const _CMD_FAKE_ERROR  = PREFIX + 'fake error';
const _CMD_AIRPOD      = PREFIX + 'airpod';
const _CMD_TECHSUPPORT = PREFIX + 'techsupport';
const _CMD_OOF         = PREFIX + 'oof';
const _CMD_FAKEPING    = PREFIX + 'fakeping';
const _CMD_DABABY      = PREFIX + 'the baby';
const _CMD_TROLLING    = PREFIX + 'trolling';
const _CMD_AMOGUS      = PREFIX + 'amogus';
const _CMD_AMONGDRIP   = PREFIX + 'amongdrip';
...
client.on('message', async (msg) => {
    try {
        ...
        } else if (msg.content.trim().toLowerCase() == _CMD_SNEEZE) {
          if (msg.member.voice.channel) {
              msg.react("815716068970332171")
        ...
        } else if (msg.content.trim().toLowerCase() == _CMD_RICKROLL) {
          if (msg.member.voice.channel) {
                msg.react("815716924246982728")
                const connection = await msg.member.voice.channel.join();
        ...
        } else if (msg.content.trim().toLowerCase() == _CMD_RINGSWOW) {
          if (msg.member.voice.channel) {
                msg.react("815716069586108528")
                const connection = await msg.member.voice.channel.join();
                const dispatcher = connection.play('./sounds/ringswow.mp3');
                dispatcher.setVolume(1);
                
              } 
        } else if (msg.content.trim().toLowerCase() == _CMD_METAL) {
          if (msg.member.voice.channel) {
                msg.react("815716069096030228")
                const connection = await msg.member.voice.channel.join();
                const dispatcher = connection.play('./sounds/metalpipefalling.mp3');
                dispatcher.setVolume(1);
                
              } 
        } else if (msg.content.trim().toLowerCase() == _CMD_JOOBIDIN) {
          if (msg.member.voice.channel) {
                msg.react("815716536558944306")
                const connection = await msg.member.voice.channel.join();
                 const ytdl = require('ytdl-core');
        ...
        } else if (msg.content.trim().toLowerCase() == _CMD_KANYE) {
          if (msg.member.voice.channel) {
                msg.react("815716924280406046")
                const connection = await msg.member.voice.channel.join();
                 const dispatcher = connection.play(ytdl('https://www.youtube.com/watch?v=mj4NWH6nihM', { filter: 'audioonly' }));
                dispatcher.setVolume(1);
                
              } 
        } else if (msg.content.trim().toLowerCase() == _CMD_FAKE_ERROR) {
          if (msg.member.voice.channel) {
                msg.react("815716536764334100")
                const connection = await msg.member.voice.channel.join();
                 const ytdl = require('ytdl-core');
        ...
        } else if (msg.content.trim().toLowerCase() == _CMD_AIRPOD) {
          if (msg.member.voice.channel) {
                msg.react("815716538559496212")
                const connection = await msg.member.voice.channel.join();
                 const ytdl = require('ytdl-core');
        ...
        } else if (msg.content.trim().toLowerCase() == _CMD_TECHSUPPORT) {
          if (msg.member.voice.channel) {
                msg.react("815724629859500042")
                const connection = await msg.member.voice.channel.join();
        ...
        } else if (msg.content.trim().toLowerCase() == _CMD_OOF) {
          if (msg.member.voice.channel) {
                msg.react("824325515208163338")
                const connection = await msg.member.voice.channel.join();
        ...
        } else if (msg.content.trim().toLowerCase() == _CMD_FAKEPING) {
          if (msg.member.voice.channel) {
                msg.react("824326431395151902")
                const connection = await msg.member.voice.channel.join();
        ...
        } else if (msg.content.trim().toLowerCase() == _CMD_DABABY) {
          if (msg.member.voice.channel) {
                msg.react("831152865841119234")
                const connection = await msg.member.voice.channel.join();
        ...
        } else if (msg.content.trim().toLowerCase() == _CMD_TROLLING) {
          if (msg.member.voice.channel) {
                msg.react("832339919828680727")
                const connection = await msg.member.voice.channel.join();
        ...
        } else if (msg.content.trim().toLowerCase() == _CMD_AMOGUS) {
          if (msg.member.voice.channel) {
                msg.react("832345729635844166")
                const connection = await msg.member.voice.channel.join();
        ...
        } else if (msg.content.trim().toLowerCase() == _CMD_AMONGDRIP) {
          if (msg.member.voice.channel) {
                msg.react("832345674992975952")
                const connection = await msg.member.voice.channel.join();
...
function getHelpString() {
    let out = '**VOICE COMMANDS:**\n'
        out += '```'
        out += 'computer help\n'
        out += 'computer play [random, favorites, <genre> or query]\n'
        out += 'computer skip\n'
        out += 'computer pause/resume\n'
        out += 'computer shuffle\n'
        out += 'computer genres\n'
        out += 'computer set favorite\n'
        out += 'computer favorites\n'
        out += 'computer list\n'
        out += 'computer clear list\n';
        out += '```'
        out += '**VOICE SOUNDBOARD COMMANDS:**\n'
        out += 'computer sneeze\n'
        out += 'computer rickroll\n'
        out += 'computer wow\n'
        out += 'computer metal\n'
        out += 'computer joobidin (NON-FUNCTIONAL FOR NOW)\n'
        out += 'computer kanye\n'
        out += 'computer win10error (NON-FUNCTIONAL FOR NOW)\n'
        out += 'computer airpod (NON-FUNCTIONAL FOR NOW)\n'
        out += 'computer tech support (NON-FUNCTIONAL FOR NOW)\n'
        out += 'computer oof (NON-FUNCTIONAL FOR NOW)\n';
        out += 'computer fake ping(NON-FUNCTIONAL FOR NOW)\n'
        out += 'computer dababy (NON-FUNCTIONAL FOR NOW)\n'
        out += 'computer trolling\n'
        out += 'computer amogus NON-FUNCTIONAL FOR NOW)\n';
         out += 'computer amongdrip (NON-FUNCTIONAL FOR NOW)\n';
        out += '```'
...
async function connect(msg, mapKey) {
    try {
        ...
        voice_Connection.play('LBT connect.mp3', { volume: 0.5 });
        guildMap.set(mapKey, {
...
function speak_impl(voice_Connection, mapKey) {
    voice_Connection.on('speaking', async (user, speaking) => {
        ...
        // starting in 1997 I don't fricking know Intel developed a new chip design to help make computing more efficient
